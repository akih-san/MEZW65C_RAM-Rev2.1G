...
#### １．CPUの高速化(6502:10MHz, 65816:8Mhz)
...
6502で10MHz、65816では8MHzで動作することが出来るようになります。<br>
<br>
（65816について補足）<br>
65816は、クロックの半周期（CLKの立ち上がり）でメモリアドレス確定し、<br>
残りの半周期（CLKの立下り）でデータを確定する仕様です。<br>
またデータ確定時（CLKの立ち下がり）のsetup timeが最低10ns必要となります。<br>
このため、アクセスタイム55nsのSRAM(AS6C4008-55PCN)では、55ns+10ns=65nsが<br>
データ確定までに必要な時間となります。<br>
このため、動作クロックの論理値は、f=1/(65ns*2)となり、[約7.69MHz](photo/w65c816%20timing%20chart.png)となります。<br>
8MHzは実力で動作しているようなので、設定ファイル（MEZW16.CFG）には8MHzで動作するように<br>
設定してあります。<br>
メモリ、CPUの個体差で動作しない場合、クロックを7MHzに下げてください。<br>
<br>
<br>
＜余談＞<br>
